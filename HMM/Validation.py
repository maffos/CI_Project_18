    ...
    for i in range(k):
        ...
        with open("Cross_Validation/data_negative.txt","a") as neg_file:
            for j in range(int((i+1)*neg_len/k), neg_len):
                neg_file.write(neg_data[j] + "\n")
#train an HMM on the training data and get the prediction on the test data
        hmm =   HMM("Cross_Validation/data_positive.txt", "Cross_Validation/data_negative.txt", "Cross_Validation/test_data.txt", "Cross_Validation/prediction.txt")
        hmm.train()
